...
namespace Dynastream.Fit
{
    ...
    public class Message
    {
        ...
        public byte LocalNum
        {
            ...
            set
            {
                if (value > Fit.LocalMessageNumMask)
                {
                    throw new FitException("Message:LocalNum - Invalid Local message number " + value + ". Local message number must be < " + Fit.LocalMessageNumMask);
                }
                else
                {
                    localNum = value;
        ...
        #region Constructors
        public Message(Message mesg)
        {
            if (mesg == null)
            {
                this.Name = "unknown";
                this.MessageNumber = (ushort)MessageNum.Invalid;
                return;
            }
            this.Name = mesg.Name;
            this.MessageNumber = mesg.MessageNumber;
        ...
        public Message(string name, ushort num)
        {
            this.Name = name;
            this.MessageNumber = num;
        }

        internal Message(ushort mesgNum)
            : this(Profile.GetMessage(mesgNum))
        {
        }
        ...
        public void SetFields(Message mesg)
        {
            if (mesg.MessageNumber != MessageNumber)
            {
                return;
            }
            foreach (Field field in mesg.FieldsList)
        ...
        public Field GetField(string fieldName, bool checkMessageSupportForSubFields)
        {
            foreach (Field field in FieldsList)
            {
                ...
                {
                    if ((subfield.Name == fieldName) && (!checkMessageSupportForSubFields || (subfield.CanMessageSupport(this))))
                    {
                        return field;
                    }
                }
        ...
        public ushort GetActiveSubFieldIndex(byte fieldNum)
        {
            ...
            {
                if (testField.subfields[i].CanMessageSupport(this))
                {
                    return i;
                }
            }
        ...
        public string GetActiveSubFieldName(byte fieldNum)
        {
            ...
            {
                if (subfield.CanMessageSupport(this))
                {
        ...
        public object GetFieldValue(byte fieldNum, int fieldArrayIndex, ushort subFieldIndex)
        {
            ...
            else
            {
                ...
                if ((subfield == null) || (subfield.CanMessageSupport(this)))
                {
                    var q = field.GetValue(fieldArrayIndex, subFieldIndex);
                    return q;
                }
                else
                {
        ...
        public void SetFieldValue(byte fieldNum, int fieldArrayIndex, Object value, ushort subfieldIndex)
        {
            ...
            if (field == null)
            {
                ...
                field = new Field(Profile.GetMessage(this.MessageNumber).GetField(fieldNum));
                if (field.Num == Fit.FieldNumInvalid)
                {
            ...
            if ((componentList != null) && (componentList.Count > 0))
            {
                foreach (FieldComponent fC in componentList)
                {
                    if (fC.fieldNum != Fit.FieldNumInvalid)
                    {
                        ...
                        Field newField = new Field(Profile.GetMessage(this.MessageNumber).GetField(fC.fieldNum));
                        ...
                        {
                            bitsValue = accumulator.Accumulate(this.MessageNumber, fC.fieldNum, bitsValue.Value, fC.bits);
                        }
