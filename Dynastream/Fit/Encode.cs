...
namespace Dynastream.Fit
{
    ...
    {
        #region Fields
        private MessageDefinition[] lastMessageDef = new MessageDefinition[Fit.MaxLocalMessages];
        ...
        public void Write(Message mesg)
        {
            if (open == false)
            {
                throw new FitException("Encode:Write - Encode not opened yet, must call Encode:Open()");
            ...
            if (!validator.ValidateMessage(mesg))
            {
            ...
            if ((lastMessageDef[mesg.LocalNum] == null) || !lastMessageDef[mesg.LocalNum].Supports(mesg))
            {
                Write(new MessageDefinition(mesg));
            }
            mesg.Write(fitDest, lastMessageDef[mesg.LocalNum]);
        }

        public void Write(IEnumerable<Message> mesgs)
        {
