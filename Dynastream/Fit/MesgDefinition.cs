...
namespace Dynastream.Fit
{
    ...
    public class MessageDefinition
    {
        ...
        #region Properties
        public ushort GlobalMessageNum { get; set; }
        public byte LocalMessageNum
        {
            get
            {
            ...
            set
            {
                ...
                {
                    throw new FitException("MessageDefinition:LocalMessageNum - Invalid Local message number " + value + ". Local message number must be < " + Fit.LocalMessageNumMask);
                }
                else
                {
                    localMessageNum = value;
                }
            }
        }
        ...
        #region Constructors
        internal MessageDefinition()
        {
            LocalMessageNum = 0;
            GlobalMessageNum = (ushort)MessageNum.Invalid;
        ...
        public MessageDefinition(Stream fitSource)
            : this(fitSource, null)
        {
        }

        public MessageDefinition(Message mesg)
        {
            LocalMessageNum = mesg.LocalNum;
            GlobalMessageNum = mesg.MessageNumber;
            architecture = Fit.LittleEndian;
        ...
        internal void Read(Stream fitSource, DeveloperDataLookup lookup)
        {
            ...
            header = br.ReadByte();
            LocalMessageNum = (byte)(header & Fit.LocalMessageNumMask);

            byte reserved = br.ReadByte();
            architecture = br.ReadByte();
            br.IsBigEndian = this.IsBigEndian;
            GlobalMessageNum = br.ReadUInt16();
            ...
            if (ContainsDevData)
            {
                ...
                for (int i = 0; i < devFldCount; i++)
                {
                    ...
                    Tuple<DeveloperDataIdMessage, FieldDescriptionMessage> tuple = lookup.GetMessages(key);
        ...
        public void Write(Stream fitDest)
        {
            ...
            byte header = LocalMessageNum;
            header |= Fit.MessageDefinitionMask;
            header |= NumDevFields > 0 ? Fit.DevDataMask : (byte)0x00;

            bw.Write(header);
            bw.Write(Fit.MessageDefinitionReserved);
            bw.Write(Fit.LittleEndian);
            bw.Write(GlobalMessageNum);
            bw.Write(NumFields);

            if (NumFields != fieldDefs.Count)
            {
                throw new FitException("MessageDefinition:Write - Field Count Internal Error");
            }
        ...
        public bool Supports(Message mesg)
        {
            return Supports(new MessageDefinition(mesg));
        }
