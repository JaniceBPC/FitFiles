...
namespace Dynastream.Fit
{
    ...
    public class Decoder
    {
        ...
        #region Fields
        private MessageDefinition[] localMessageDefs = new MessageDefinition[Fit.MaxLocalMessages];
        ...
        private void RaiseMessageEvent(Message newMessage)
        {
            if ((newMessage.MessageNumber == MessageNum.DeveloperDataId) ||
                (newMessage.MessageNumber == MessageNum.FieldDescription))
            {
                HandleMetaData(newMessage);
            }

            if (MessageEvent != null)
            {
                MessageEvent(this, new MessageEventArgs(newMessage));
            }
        }

        private void HandleMetaData(Message newMessage)
        {
            if (newMessage.MessageNumber == MessageNum.DeveloperDataId)
            {
                var mesg = new DeveloperDataIdMessage(newMessage);
                m_lookup.Add(mesg);
            }
            else if (newMessage.MessageNumber == MessageNum.FieldDescription)
            {
                var mesg = new FieldDescriptionMessage(newMessage);
